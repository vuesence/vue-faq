...
основными задачами? Несомненно, большой вклад в эту ситуацию внесли
...
В этой главе мы расскажем о том, что такое человеко-компьютерное
взаимодействие.

В этой главе мы сделаем шаг назад от тонкостей внутренних паттернов и
архитектуры программного обеспечения и рассмотрим некоторые из
устоявшихся паттернов для проектирования **пользовательского
интерфейса** (**UI**) и **пользовательского опыта** (**UX**). Мы сделаем
следующее:

-   Поймем различия между UI и UX
-   Познакомимся с распространенными паттернами UX в веб-приложениях
-   Узнаем о злой стороне UX с помощью темных паттернов

К концу этой главы вы получите широкое представление о том, что такое
UX-паттерны, на каких принципах они основаны, каковы наиболее
...
<https://github.com/PacktPublishing/Vue.js-3-Design-Patterns-and-Best-Practices/tree/main/Chapter11>.

Посмотрите следующее видео, чтобы увидеть код в действии:
<https://packt.link/5ymkr>
...
-   **Дизайн пользовательского интерфейса** отвечает за определение
    ...
    аппаратным обеспечением). Это относится как к визуальным средам
...
-   **UX-дизайн** включает условия, которые влияют на дизайн
    ...
    функции, с которой он взаимодействует. Он включает в себя не только
    ...
    зрения пользователя. Под этим определением подразумевается
    ...
    бренда и т.д. Его основная цель - изменить или создать у
...
становились на путь птицы дронта, поскольку пренебрегали
...
продукта. Для наших целей мы ограничим рассмотрение пользовательских
...
В настоящее время существует большое количество источников информации о
...
видеть вещи иначе, чем промышленный дизайнер. В нашем случае большинство
паттернов для проектирования пользовательского интерфейса с самого
начала были включены в стандарт HTML, поэтому большинство, если не все,
из того, что мы будем рассматривать как паттерны, уже знакомо вам и
...
очевидным. Например, почему символ "X" для закрытия окна находится в
...
интерфейса. Рассмотрим в качестве примера домашнюю страницу Packt
(<https://www.packtpub.com/>):

![](/book/images/Figure_11.01_B18602.jpg)
width="519" height="174"
alt="Рисунок 11.1 - Главная страница Packt и цветовой контраст" />.
...
привлекает внимание пользователя: это кнопки **TRY FOR FREE** и **START
LEARNING NOW**. Очевидно, что дизайнер установил точку фокусировки на
этих двух действиях.
...
более или менее одинаковое повторяющееся поведение. Например, если вы
...
width="1124" height="557" alt="Рисунок 11.2 - Карточки с книгами" />.
...
Короче говоря, это повторение визуального дизайна.
...
Другой пример - выравнивание вкладки.
...
alt="Рисунок 11.3 - Футер сайта Packt, использующий выравнивание для отображения естественных групп" />
...
width="380" height="98" alt="Рисунок 11.4 - Значки абзацев" />.
...
*Композиция пользовательского интерфейса с компонентами*. Если же вы
...
законе можно найти в Википедии:
...
3.  Предлагать информативную обратную связь (особенно при возникновении
    ошибок).
4.  Дизайн диалогов с окончанием.
5.  Предлагать простую обработку ошибок, чтобы пользователь мог быстро
...
6.  Позволяют легко и просто выполнять действия, связанные с удалением
    содержимого.
7.  Позволяют легко отменять действия (благодаря этому у нас есть
    *Ctrl* + *Z*!).
8.  Поддерживать чувство контроля у пользователя. Нет ничего хуже для
...
9.  Сокращение времени на печать.
10. Сокращение кратковременной памяти. Пользователь может одновременно
...
### Принципы проектированияUX

UX-дизайн также имеет свои цели и принципы, которые распространяются на
паттерны. Прежде всего, основная цель UX - обеспечить хорошее восприятие
...
происходит с мрачными шаблонами, которые мы рассмотрим далее в этой
...
поддерживаемых современными браузерами, можно найти здесь:
<https://developer.mozilla.org/en-US/docs/Web/HTML/Element/Input#input_types>.

По большей части эти элементы используются с базовой функциональностью и
некоторым тяжелым CSS-стилем. Используйте **вводы** (и **текстовые
области**) в тех случаях, когда пользователю необходимо ввести текстовую
...
## Чекбоксы, радиоприемники и тумблеры
...
**Клавиши** представляют пользователю несколько вариантов, которые он
...
width="820" height="187"
alt="Рисунок 11.5 - Checkboxes слева и radio buttons справа" />.

Рисунок 11.5 - Флажки слева и радиокнопки справа

С выходом оригинального iPhone стал очень популярен новый вариант
флажка: **тумблер**. Он не предусмотрен стандартом HTML, но его можно
легко стилизовать с помощью CSS, чтобы "замаскировать" флажок. Тумблер
...
width="606" height="103"
alt="Рисунок 11.6 - Тумблеры в каждом состоянии" />.
...
Этот паттерн включает в себя короткий текст (или "копию" на языке
...
действием, например, символом "**X**" для ее удаления. Обычно пилюли
используются в списках для отображения атрибутов, тегов, категорий или
других типов классификаторов для данного элемента. В некоторых случаях
они могут использоваться и для выбора (переключения) элементов из
списка. В этом случае они конкурируют или ведут себя так же, как флажок,
...
width="530" height="98"
alt="Рисунок 11.7 - Список элементов с таблетками" />.
...
# Общие паттерны для визуализации данных
...
Этот паттерн показывает пользователю плавающий небольшой текст с
...
width="289" height="70"
alt="Рисунок 11.8 - Подсказка, отображающая название/действие пиктограммы и ярлык" />.
...
width="546" height="117"
alt="Рисунок 11.9 - Шаблон всплывающей подсказки, используемый для отображения контекстного меню" />.
...
Контекстное меню - это не просто меню, а меню, которое открывается при
выборе или предварительной активации.

## Уведомляющие точки, пузырьки, метки или значки

.

Этот паттерн заключается в отображении маленького значка поверх более
...
также называют **значком.** Ниже приведены примеры этого шаблона:

-   Маленький кружок с цифрой для новых полученных писем в значке
    электронной почты
-   Двойная галочка в приложении чата, указывающая на то, что собеседник
    получил и прочитал сообщение
-   Маленькая отметка на панели задач, показывающая, что приложение было
    открыто
...
Этот паттерн используется во многих приложениях и ОС. Он заключается в
...
сочетании с понятной пиктограммой. В зависимости от ОС оно может
...
width="524" height="121"
alt="Рисунок 11.10 - Нативное уведомление о тосте на рабочем столе" />.
...
торговые и новостные сайты используют этот паттерн - например,
...
width="481" height="150"
alt="Рисунок 11.11 - Карусель Amazon отображает предложения от магазина" />.
...
width="481" height="97"
alt="Рисунок 11.12 - Amazon хорошо использует слайдеры изображений для отображения товаров пользователю" />.
...
выполнения ограниченного числа действий. Основная цель - уведомить
...
наглядность и чувство контроля для пользователя. Прогресс-бары очень
...
width="524" height="210"
alt="Рисунок 11.13 - Пример индикатора выполнения" />.

Рисунок 11.13 - Пример полос прогресса
...
с подобными ситуациями (элемент progress; см.
...
width="497" height="102"
alt="Рисунок 11.14 - Круг спиннера, указывающий на занятость приложения" />.
...
использовании этого паттерна - после длительного использования он может
...
<div class="orm-ChapterReader-codeSnippetContainer"
testid="custom pre block">
...
<шаблон>
    <div>
         <span class="спиннер"></span> {{ $props.caption }}
    </div>
</template>
<style scoped>
.spinner{
    display: inline-block;
    высота: 1rem; ширина: 1rem;
    border: 2px solid;
    vertical-align: middle;
    border-radius: 50%;
    border-top-color: #06c9c9;
    анимация: rotate 1s linear infinite;
}
@keyframes rotate {
...
<div class="orm-ChapterReader-snippetButtonContainer">

В этом простом компоненте нам нужно определить только prop для текста и
...
width="713" height="188"
alt="Рисунок 11.15 - Элементы типичного пейджера" />.
...
Альтернативой пейджеру может служить его страница.

Альтернативой пагинации является использование **бесконечного
скроллера**. В этом случае точное местоположение каждого элемента в
...
width="711" height="417"
alt="Рисунок 11.16 - Реализация бесконечного скроллера" />.
...
реализаций - использование **наблюдателя пересечения**, собственного
...
width="681" height="175"
alt="Рисунок 11.17 - Пример бесконечного скроллера, ограниченного элементом div" />.
...
больше содержимого (как на *Рисунок 11.17*). Теперь техника заключается
...
этапе. В строке **\#3** мы создаем новый **IntersectionObserver** и
...
элементом scroll watcher. Волшебство происходит в строке **\#4**, когда
...
области просмотра, что произойдет, когда мы дойдем до конца списка. В
этой функции мы снова увеличиваем количество элементов, заставляя Vue
...
Помимо реализации UX-паттерна, данная техника является правильным
...
# Общие паттерны для взаимодействия и навигации

Эти паттерны управляют взаимодействием или предлагают пользователю
варианты управления процессом и навигацией по приложению. Как это обычно
бывает, некоторые из этих паттернов могут быть отнесены и к другим
категориям.

## Размещение меню

Размещение меню - это тоже паттерн, который стандартизирован в трех
основных вариантах:

-   **Панели меню** (горизонтальные) часто размещаются в "липкой"
    позиции в верхней части экрана (то есть они не прокручиваются вместе
    со страницей).
-   **Панели навигации** (горизонтальные), популярные в основном для
    ...
    приложения.
-   **Боковые панели** занимают всю высоту экрана и имеют переменную
...
Следование стандартному расположению пиктограмм для настольных и
...
``` source-code
Главная > Уровень 1 > Уровень 2 > Уровень 3 > Текущая страница
```
...
more-than (**\>**), однако в этом вопросе существует множество
художественных лицензий. Другим вариантом представления такой навигации
...
навигации, а в основном используется для вложенного контента, например,
...
пор, пока не будет решена задача, представленная в диалоге. Модальные
...
width="533" height="110" alt="Рисунок 11.19 - Иконки меню" />.
...
-   **Значок гамбургера** (три горизонтальные линии): Зарезервированный
...
width="855" height="383"
alt="Figure 11.20 - Accordion menu used for FAQ" />.

Рисунок 11.20 - Аккордеонное меню, используемое для FAQ

.

Меню в виде аккордеона - это хорошо понятный пользователям паттерн,
...
До сих пор большинство рассмотренных нами паттернов преследовали цель
скрыть сложность от пользователя, чтобы не перегружать его. Однако
данный паттерн, на первый взгляд, нарушает это правило. Если сложность
...
использование **мега-меню**. Этот паттерн часто используется на
...
выбора. Это не означает, что после достижения этих разделов не будет
"сверления вниз", но это делает доступ более быстрым. Например,
...
как место для других паттернов, например, боковой панели или
меню-аккордеона. На официальном сайте Packt
(<https://www.packtpub.com/>) этот паттерн используется в главном меню
...
width="1428" height="567"
alt="Рисунок 11.22 - Мега меню Packt с боковой панелью для категоризации и фильтрации опций" />.
...
конкретная реализация. Эту концепцию используют, например, аккордеонное
...
# Общие шаблоны для списков выбора

# Общие паттерны для организации контента

Следующие паттерны связаны с общей организацией и компоновкой сайта или
приложения.

## Резонансные приложения
...
настольного компьютера, который считается самым большим. Хотя существуют
...
В зависимости от того, как это будет реализовано, в зависимости от того,
...
только одну из них - метод "переключения колонок". Этот метод, по сути,
...
width="529" height="291"
alt="Рисунок 11.23 - Трансформация с первого мобильного на рабочий стол" />.
...
только вы поймете этот паттерн, вы начнете видеть его повсюду, где он
применяется.

Простой подход к кодированию.
...
Этот паттерн настолько распространен, что мы даже не задумываемся о нем.
...
тем, как пользователи "сканируют" страницу. Разные пользователи при
...
alt="Рисунок 11.24 - Домашняя страница Google является исключением из этого шаблона, поскольку логотип не является ссылкой" />.
...
width="588" height="155"
alt="Рисунок 11.25 - Результат поиска Google применяет шаблон" />.
...
## Главная секция, призыв к действию и иконки социальных сетей

Раздел **героя** - это первая часть, которая отображается при загрузке
...
содержимое, расположенное ниже этого раздела, называется "ниже сгиба",
т.е. чтобы увидеть его, пользователю необходимо прокрутить страницу
вниз. Раздел-герой считается наиболее важной частью главной страницы и
местом, где размещается первоначальный **призыв к действию**. Мы уже
...
width="480" height="184"
alt="Рисунок 11.26 - Героический раздел главной страницы Kobold Press - красочный и прямой" />.

Рисунок 11.26 - Героический раздел главной страницы Kobold Press -
красочный и непосредственный

На предыдущем рисунке, который мы приводим в качестве примера, герой
главной страницы Kobold Press - красочный и непосредственный.
...
призывом к действию. Обратите внимание также на то, что слева в виде
...
Есть и другие паттерны, сотни, если не тысячи, которые оказывают
...
-   Паттерны регистрации и снятия с регистрации
...
Темные паттерны - это тщательно продуманные интерфейсы и взаимодействия
...
паттернами проектирования, многие из них пересекаются или могут быть
вложены друг в друга. Давайте рассмотрим их по очереди.

## Вопросы с подвохом

Этот паттерн представляет собой простую или сложную игру слов,
...
width="663" height="100"
alt="Рисунок 11.27 - Вопрос-обманка для подписки на рассылку" />
...
Этот темный паттерн встречается во многих приложениях для покупок,
...
width="682" height="104"
alt="Рисунок 11.28 - В корзину был добавлен дополнительный товар" />.
...
Этот паттерн довольно часто встречается для услуг и подписок. Он
...
Цена скрывается или маскируется.
...
Это еще один паттерн, часто используемый в рабочих процессах корзины. Он
...
паттерн часто используется для манипулирования пользователем с целью
...
width="706" height="130"
alt="Рисунок 11.29 - Диалог подтверждения при удалении услуги из корзины" />.
...
паттерн является обоюдоострым мечом, так как пользователь может
...
кнопок **Загрузить** на странице, но только одна из них действительно
...
width="1176" height="356"
alt="Рисунок 11.30 - FileHippo.com размещает бесплатные программы. Некоторые объявления маскируются под кнопки загрузки" />
...
В списке темных шаблонов есть и такой.

Перечень темных паттернов может не быть напрямую связан с каким-либо
...
положительного опыта. Мы также рассмотрели некоторые из наиболее
...
-   Что такое UI и UX паттерны?
-   Можете ли вы назвать преимущества использования паттернов UX?
-   Какую пользу может принести использование стандартных паттернов в
    компонентах Vue 3?
-   Что такое темные паттерны и почему их следует избегать?
