...
В VS Code намного раньше появляются новые официальные расширения для Vue, и вообще эта среда очень удобна для разработки, есть множество полезных расширений, - например, запустить GitHub Action и посмотреть его вывод прямо в редакторе.
