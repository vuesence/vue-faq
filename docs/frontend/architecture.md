...
- Использовать обертки над компонентами UI библиотек (Оборачивать компоненты/утилиты в обертки)
...
- Старайтесь использовать для адаптивноcти чистый CSS3. Не привязывайтесь к колоночным моделям CSS и UI библиотек, если только это не проект из разряда: "сделал и забыл" Не покупайтесь на ТВ, потратьте пару дней на изучение флексбокса
- Используйте семантические элементы HTML5 - aside, header, section, article, details и.т.д
- Старайтесь документировать проект, рисуя use-case диаграммы, описывая его основной функционал и важные детали реализации - это даст вам рост как разработчику ПО. VitePress - очень удобный инструмент для этого.
